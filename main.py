...
class Game:
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                if event.type == QUIT:
                    ...
                    grid_pos = tuple(map(lambda x: int(x / BLOCK_SIZE), pygame.mouse.get_pos()))
                    self.placeFood(Vector2(grid_pos))
            ...
            for i, food in enumerate(self.foods):
                ...
                if self.player.pos.x == food.pos.x and self.player.pos.y == food.pos.y:
                    ...
                    Renderer.drawFood(self.screen, food)
    ...
    def placeFood(self, pos=None):
        if pos is None:
            space_free = False
            while not space_free:
                pos = Helper.randomPos(self.area)
                space_free = True
                for body_part in self.player.tail:
...
class Renderer():
    ...
    def drawFood(screen, food):
        left = food.pos.x * BLOCK_SIZE
        top = food.pos.y * BLOCK_SIZE
        offs = max(0, BLOCK_SIZE/2 - food.age)
        rect = Rect(left + offs, top + offs, BLOCK_SIZE - offs*2, BLOCK_SIZE - offs*2)
        pygame.draw.ellipse(screen, food.color, rect)
